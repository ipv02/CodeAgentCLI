...
и отвечает по локальному индексу проектной документации с проверенными
...
локального retrieval и ответа используйте `agent --local-context-chat`.
...
### Файловый ассистент в обычном agent

Обычный `agent` распознаёт цели, требующие работы с файлами проекта. Отдельный
режим запускать не нужно. Маршрутизация срабатывает по файловой формулировке или
по явно указанным путям вроде `docs/guide.md` и `project_files.py`:

```bash
agent "Найди все места, где используется SupportAssistantService в файлах проекта"
agent "Изучи MCP-реализацию и создай docs/adr/project-files.md"
agent "Проверь README по текущему main.py и подготовь исправления"
```

Ассистент сам вызывает встроенный `project-files` MCP: перечисляет файлы,
ищет совпадения, читает 2–3 релевантных файла и анализирует их. Поиск выводит
`файл:строка`. Создание или обновление сначала возвращает читаемый diff и ничего
не записывает. Для существующего файла модель предлагает точечные замены
`old → new`, поэтому большой документ не нужно генерировать заново целиком.

В интерактивном `agent` подготовленное изменение управляется командами:
...
`/files diff` показывает спокойный цветной preview без фоновой заливки: зелёный
gutter отмечает новые и добавленные строки, красный — удалённые, а само
содержимое остаётся обычным светлым текстом. При перенаправлении вывода или
`NO_COLOR=1` остаются текстовые подписи «Добавлено», «Удалено» и «Контекст».
Технический unified diff со служебными `@@`, `+++` и `---` доступен через
`/files raw-diff`.

Для воспроизводимой one-shot записи требуется явный флаг:

```bash
agent --apply "Изучи MCP-реализацию и создай docs/adr/project-files.md"
```

Файловый сервер можно зарегистрировать в постоянном MCP config:
...
Все пути ограничены корнем проекта. Доступ к `.git`, `.venv`, локальному state,
секретам и symlink запрещён. Перед записью проверяется SHA исходного файла,
поэтому параллельное изменение не будет молча перезаписано. При генерации через
DeepSeek выбранные фрагменты файлов отправляются внешнему API; содержимое файлов
считается недоверенными данными.

Подробности и воспроизводимые сценарии: [`docs/project-file-assistant.md`](docs/project-file-assistant.md).
...
Во время подключения к MCP-серверам команда показывает loader
`Получаю список MCP tools...`, затем выводит состояние каждого сервера и tools.
...
При аргументе `.` агент использует текущий корень проекта. Если editable-версия
CodeAgentCLI запущена из другого каталога, корень репозитория определяется
автоматически. Для явного выбора используйте абсолютный путь или переменную
`CODE_AGENT_PROJECT_DIR`.
...
Для ускорения Ollama получает embeddings пакетами; размер пакета задаётся через
`CODE_AGENT_EMBED_BATCH_SIZE` и по умолчанию равен `32`.
